        ...
        for idx, item in enumerate(items, 1):
            ...
            if ftype == "pdf":
                ...
                if ok and dest.exists() and dest.stat().st_size > 0:
                    ...
                    fail_count += 1
                    await client.send_message(chat_id, f"❌ Download failed: `{name}`", parse_mode="markdown")
            ...
            # ── VIDEO ──
                ...
                if ok and dest.exists() and dest.stat().st_size > 0:
                    try:
                        prog_msg = await client.send_message(chat_id, f"📤 Uploading: `{name}`...", parse_mode="markdown")
                        last_edit = [time.time()]
