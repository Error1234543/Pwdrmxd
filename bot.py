...
async def download_video(proxy_url: str, dest: Path, prog_msg, name: str) -> bool:
    try:
        ...
        # Step 1: Get real video size via ffprobe
        total_size = 0
        try:
        ...
        async def watcher():
            ...
            while proc.returncode is None:
                ...
                if dest.exists():
                    ...
                    speed   = done / elapsed if elapsed > 0 else 0
                    # Use real total if available, else use done as total
                    display_total = total_size if total_size > 0 else done
                    now = time.time()
                    if now - last_edit > 6:
                        last_edit = now
                        try:
                            await prog_msg.edit_text(
                                progress_box(name, done, display_total, speed, elapsed, "📥 Downloading Video"),
                                parse_mode="Markdown"
                            )
                        except Exception:
                            pass
        ...
        if proc.returncode != 0:
            log.error(f"ffmpeg error: {stderr.decode()[-300:]}")
            return False
        return True
...
async def recv_file(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await msg.edit_text(
        ...
        f"Example: `25` bhejo → *25 se {total} tak* download hoga\n"
        f"`1` bhejo → *sab* ({total} links) download hoga",
        parse_mode="Markdown"
    )
...
async def recv_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "wait_start_num":
        ...
        try:
            ...
            await update.message.reply_text(
                f"❌ *1 se {total}* ke beech number bhejo.\nExample: `25`",
                parse_mode="Markdown"
            )
            return
        ...
        await update.message.reply_text(
            ...
            f"🔑 Apna *PW Token* bhejo (`eyJ...` se shuru hoga):",
            parse_mode="Markdown"
        )
        return
    ...
    if step == "wait_token":
        if len(text) < 50 or not text.startswith("eyJ"):
            await update.message.reply_text(
                "❌ Invalid token\\. `eyJ...` se shuru hona chahiye\\.",
                parse_mode="MarkdownV2"
            )
            return
        ...
        await update.message.reply_text("🎬 *Video quality select karo:*", reply_markup=kb, parse_mode="Markdown")
        return
    ...
    if step == "wait_caption":
        ...
        await update.message.reply_text(
            f"✅ Caption: `{text}`\n\n"
            f"🚀 *Download + Upload shuru ho raha hai...*\n"
...
async def cb_quality(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    state = sessions.get(uid)
    if not state:
        await query.answer("Session expire ho gaya. /start karo.", show_alert=True)
        return
    ...
    await query.message.edit_text(
        f"✅ Quality: *{quality}p*\n\n"
        f"📝 *Caption prefix bhejo* (videos ke liye)\n"
...
async def process_all(bot, chat_id: int, uid: int):
    ...
    for idx, item in enumerate(selected, 1):
        ...
        url       = item["url"]
        ftype     = item["type"]
        real_num  = start_from + idx - 1   # original file number
        caption   = (
            f"*{prefix}*\n\n"
            f"📌 `{name}`\n"
            f"🔢 `#{real_num}`\n\n"
        ...
        if ftype == "pdf":
            ...
            if ok and dest.exists() and dest.stat().st_size > 0:
                ...
                try:
                    ...
                    await prog_msg.edit_text(f"✅ `{name}`", parse_mode="Markdown")
                    ok_cnt += 1
                except Exception as e:
                    log.error(f"PDF upload: {e}")
                    await prog_msg.edit_text(f"❌ Upload failed: `{name}`", parse_mode="Markdown")
                    fail_cnt += 1
                finally:
...
def main():
    ...
    log.info(f"Webhook start → {WEBHOOK_URL}  port={PORT}")
    app.run_webhook(
        listen="0.0.0.0",
        port=PORT,
