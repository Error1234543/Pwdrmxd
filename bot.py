...
def parse_links_file(text):
    ...
    for line in text.strip().splitlines():
        ...
        if idx != -1:
            name, url = line[:idx].strip(), "https://"+line[idx+9:].strip()
        else:
    ...
    for block in blocks:
        ...
        for line in lines[1:]:
            if re.match(r'^\(\d+\)', line):
                ...
                try: answer = int(line.split(":")[1].strip()) - 1
                except: pass
...
async def safe_send(bot, chat_id, text, retries=5):
    for i in range(retries):
        ...
        except Exception as e: logger.warning(f"Send fail({i+1}): {e}"); await asyncio.sleep(3)
    return None
...
async def safe_send_doc(bot, chat_id, filepath, filename, caption, retries=5):
    for i in range(retries):
        ...
        except (TimedOut,NetworkError) as e: logger.warning(f"Upload net({i+1}):{e}"); await asyncio.sleep(10)
        except Exception as e: logger.error(f"Upload fail({i+1}):{e}"); await asyncio.sleep(5)
    return None
...
# ══════════════════════════════════════════════════════════════════════════════
#  HTML QUIZ GENERATOR — Same dark theme as index.html
# ══════════════════════════════════════════════════════════════════════════════
def build_quiz_html(title: str, questions: list) -> str:
    """Build standalone quiz HTML same as index.html dark theme"""
    q_json_list = []
    for (q, opts, ans) in questions:
        # Escape quotes for JSON safety
        safe_q = q.replace("\\","\\\\").replace('"','\\"').replace("\n"," ")
        safe_opts = [o.replace("\\","\\\\").replace('"','\\"').replace("\n"," ") for o in opts]
        opts_json = "[" + ",".join(f'"{o}"' for o in safe_opts) + "]"
        q_json_list.append(f'{{"q":"{safe_q}","opts":{opts_json},"ans":{ans}}}')
    q_json = "[" + ",".join(q_json_list) + "]"
...
let userAns={{}},curIdx=0,timerIv=null;
...
function renderQ(){{
  ...
  const explainHtml=done?
    '<div class="explain-box">✓ Sahi Jawab: <strong style="color:#fff">'+LETTERS[q.ans]+'. '+q.opts[q.ans]+'</strong></div>':'';
  document.getElementById('q-area').innerHTML=
    '<div class="question-card"><div class="q-header"><span class="q-num">Q'+(curIdx+1)+'</span>'+
    '<div class="q-text">'+q.q+'</div></div>'+
    '<div class="options-grid">'+optHtml+'</div>'+explainHtml+'</div>';
}}
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not is_authorized(uid):
        await update.message.reply_text("❌ Aap authorized nahi hain."); return
    ...
    if str(uid) in resume:
        ...
        keyboard.append([InlineKeyboardButton(
            f"▶️ Resume Download (#{idx+1}, {total-idx} left)",
...
async def button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if query.data == "mode_download":
        ...
        await query.edit_message_text(
            "🌐 *Quiz HTML Mode*\n\n"
            "MCQ paste karo — Dark theme HTML quiz file milegi!\n\n"
            "*Format:*\n`Q1. Question`\n`(1) A`\n`(2) B`\n`(3) C`\n`(4) D`\n`ANS: 2`\n\n"
...
async def handle_document(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not entries:
        await msg.edit_text("❌ Koi valid link nahi mili!\n\n*Format:*\n`Name:https://url.pdf`",
...
async def handle_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── HTML mode: MCQ paste ──
    if mode == "quiz_html" and sess.get("waiting_mcq"):
        if "ANS:" not in text.upper():
            await update.message.reply_text("⚠️ `ANS:` line nahi mili. Format check karo.",
                                             parse_mode=ParseMode.MARKDOWN); return
        await handle_quiz_html(update, ctx, text, sess.get("title","Quiz"))
        user_sessions.pop(uid, None)
        return
    ...
    if mode == "quiz_poll":
        if "ANS:" not in text.upper():
            await update.message.reply_text("⚠️ `ANS:` line nahi mili.", parse_mode=ParseMode.MARKDOWN); return
        await handle_quiz_poll(update, ctx, text)
        return

    # ── Download mode: number input ──
...
async def handle_quiz_poll(update: Update, ctx: ContextTypes.DEFAULT_TYPE, text: str):
    ...
    for idx,(q,opts,ans) in enumerate(questions):
        try:
            ...
            success += 1
            if (idx+1)%5==0:
                await safe_edit(msg, f"⏳ *{idx+1}/{len(questions)} polls sent...*")
...
async def handle_quiz_html(update: Update, ctx: ContextTypes.DEFAULT_TYPE, text: str, title: str):
    ...
    msg = await update.message.reply_text(
        f"⚙️ *{len(questions)} questions* se HTML ban rahi hai...",
        parse_mode=ParseMode.MARKDOWN)
    ...
    try:
        ...
        logger.error(f"HTML gen error: {e}")
        await safe_edit(msg, f"❌ HTML banane mein error: `{str(e)[:100]}`")
...
async def run_downloads(uid, ctx, chat_id, start_idx):
    ...
    for i in range(start_idx, total):
        ...
        try:
            ...
            if not ok:
                await safe_send(ctx.bot, chat_id, f"⚠️ *Skip #{i+1}* (download fail)\n`{name[:50]}`"); continue
            ...
            logger.error(f"Error at #{i+1}: {e}", exc_info=True)
