...
async def download_video(proxy_url: str, dest: Path, prog_msg, name: str) -> bool:
    try:
        ...
        # ffmpeg download - optimized for speed
...
async def recv_file(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await msg.edit_text(
        ...
        f"• 25 bhejo → 25 se {total} tak download hoga\n"
...
async def cb_quality(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    await query.message.edit_text(
        f"✅ Quality: {quality}p\n\n"
        f"📝 Batch name bhejo (caption ke liye)\n"
        ...
        # ── VIDEO ──
            ...
            if ok and dest.exists() and dest.stat().st_size > 0:
                ...
                try:
                    ...
                    async def upload_progress(current, total_b):
                        now = time.time()
                        if now - last_edit2[0] > 5:
                            ...
                            try:
                                await prog_msg.edit_text(
                                    progress_box(name, current, fsize, speed, elapsed, "📤 Uploading Video")
                                )
                            except Exception:
                                pass
                    ...
                    log.error(f"Video upload error: {e}")
                    # Try as document if video upload fails
                    try:
                        await prog_msg.edit_text(f"⚠️ Retry as document: {name}")
                        with open(dest, "rb") as fh:
                            await bot.send_document(
                                chat_id,
                                document=fh,
                        ...
                        await prog_msg.edit_text(f"✅ Done (as doc): {name}")
                        ok_cnt += 1
                    except Exception as e2:
                        log.error(f"Doc upload also failed: {e2}")
                        await prog_msg.edit_text(f"❌ Upload failed: {name}\n{str(e2)[:100]}")
                        fail_cnt += 1
                finally:
                    dest.unlink(missing_ok=True)
