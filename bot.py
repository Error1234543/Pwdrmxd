...
async def button_handler(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── AI Test Generator flow (NEET / GSEB / JEE Mains / GUJCET) ──
        ...
        await query.edit_message_text(
            ...
            f"Har topic ke liye alag-alag {ai_prefs[uid]['count']} questions banenge, sab ek hi HTML file mein.\n"
...
async def run_ai_generation(uid, ctx, chat_id, topics, prefs):
    ...
    main_title = topics[0] if len(topics) == 1 else f"{exam_label} Mixed Test"
    total_topics = len(topics)

    msg = await safe_send(ctx.bot, chat_id,
        f"⏳ *Generating {count} questions/topic × {total_topics} topic(s)...*\n"
        f"🏫 Exam: *{exam_label}*\n🎯 Level: *{level.title()}*\n🌐 Language: *{language.title()}*\n\n"
        f"_Please wait, ye kaafi topics/questions ke saath thoda time le sakta hai..._")
    ...
    try:
        for t_idx, topic in enumerate(topics):
            ...
            for q in questions:
                all_quiz_qs.append({
                    ...
                    "ref": q.get("ncert_ref", ""),
                    "chapter": topic if total_topics > 1 else "",
                })
        ...
        html_content = build_quiz_html(main_title, all_quiz_qs)
        safe_name = re.sub(r'[^\w\s\-.]', '', main_title).strip().replace(' ', '-')[:80] or "test"
        filename = f"{safe_name}.html"
        filepath = DOWNLOAD_DIR / filename
        async with aiofiles.open(filepath, "w", encoding="utf-8") as f:
            await f.write(html_content)

        topics_preview = ", ".join(topics[:5]) + (f" +{len(topics)-5} more" if len(topics) > 5 else "")
        caption = (f"✅ *{len(all_quiz_qs)} questions* ready across *{total_topics} topic(s)*!\n\n"
