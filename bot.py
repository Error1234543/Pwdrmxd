    ...
    try:
        async with aiohttp.ClientSession() as s:
            async with s.get(url, timeout=aiohttp.ClientTimeout(total=600)) as r:
                if r.status != 200:
                    log.error(f"PDF HTTP {r.status} for {url}")
                    return False
...
async def process_all(bot, chat_id: int, uid: int):
    ...
    for idx, item in enumerate(selected, 1):
        ...
        # ── PDF ──
        if ftype == "pdf":
            # idx prefix ensures unique filename even if names are same
                ...
                try:
                    ...
                    with open(dest, "rb") as fh:
                        await bot.send_document(
                            chat_id,
                            document=fh,
                            filename=f"{fname}.pdf",   # ← original name as filename
                            caption=caption,
                            read_timeout=300,
                            write_timeout=300,
                            connect_timeout=60,
        ...
        # ── VIDEO ──
            ...
            if ok and dest.exists() and dest.stat().st_size > 0:
                ...
                async def up_progress(current, total_b):
                    now = time.time()
                    if now - last_up[0] > 5:
                        last_up[0] = now
                ...
                try:
                    ...
                    try:
                        await prog_msg.edit_text(f"⚠️ Video retry as document: {name}")
                        with open(dest, "rb") as fh:
                            await bot.send_document(
                                chat_id,
                                document=fh,
                        ...
                        await prog_msg.edit_text(f"✅ Done (doc): {name}")
                        ok_cnt += 1
                    except Exception as e2:
                        log.error(f"Doc fallback failed: {e2}")
                        await prog_msg.edit_text(f"❌ Upload failed: {name}\n{str(e2)[:100]}")
                        fail_cnt += 1
                finally:
                    dest.unlink(missing_ok=True)
