...
async def generate_notes(topic, language):
    if language == "gujarati":
...
async def run_ai_generation(uid, ctx, chat_id, topics, prefs):
    ...
    msg = await safe_send(ctx.bot, chat_id,
        f"⏳ *Generating {count} questions/topic × {total_topics} topic(s)...*\n"
        f"🏫 Exam: *{exam_label}*\n🎯 Level: *{level.title()}*\n🌐 Language: *{language.title()}*\n\n"
        f"_Har topic ki alag HTML file aayegi. Please wait..._")

    sent_count = 0; failed_topics = []
    try:
        for t_idx, topic in enumerate(topics):
            async def on_progress(done, total, batch, total_b, _t_idx=t_idx, _topic=topic):
                pct = int(done/total*100) if total else 0
                bar = "█"*min(pct//5,20) + "░"*max(0,20-pct//5)
                await safe_edit(msg,
                    f"⏳ *Topic {_t_idx+1}/{total_topics}: {_topic[:40]}*\n"
                    f"`[{bar}] {pct}%`  ({done}/{total} questions)\n"
                    f"📦 Batch {batch}/{total_b}  |  📁 Sent so far: {sent_count}\n\n"
            ...
            safe_name = re.sub(r'[^\w\s\-.]', '', topic).strip().replace(' ', '-')[:80] or f"topic-{t_idx+1}"
            filename = f"{safe_name}.html"
        ...
        summary = f"🎉 *Done!* {sent_count}/{total_topics} topic file(s) bheji gayi."
        if failed_topics:
            summary += f"\n\n❌ Fail ho gaye ({len(failed_topics)}):\n" + "\n".join(f"• {t}" for t in failed_topics[:10])
        await safe_edit(msg, summary)
    except asyncio.CancelledError:
        await safe_edit(msg, f"⏹️ *Generation stopped.* {sent_count}/{total_topics} files bhej chuke the.")
    except Exception as e:
...
async def run_notes_generation(uid, ctx, chat_id, topics, prefs):
    language = prefs.get("language", "english")
    main_title = topics[0] if len(topics) == 1 else "Study Notes"

    msg = await safe_send(ctx.bot, chat_id,
        f"⏳ *Notes ban rahi hain...*\n📚 Topics: *{', '.join(topics)}*\n🌐 Language: *{language.title()}*\n\n_Please wait..._")

    all_notes = []
    try:
        for idx, topic in enumerate(topics):
            await safe_edit(msg, f"⏳ *Generating notes {idx+1}/{len(topics)}:* {topic}\n\n_Please wait..._")
            notes = await generate_notes(topic, language)
            if notes:
                all_notes.append(notes)

        if not all_notes:
            await safe_edit(msg, "❌ *Notes generate nahi ho sakin.* Dobara try karo ya topic thoda specific likho.")
            return

        html_content = build_notes_html(main_title, all_notes)
        safe_name = re.sub(r'[^\w\s\-.]', '', main_title).strip().replace(' ', '-')[:80] or "notes"
        filename = f"{safe_name}-Notes.html"
        filepath = DOWNLOAD_DIR / filename
        async with aiofiles.open(filepath, "w", encoding="utf-8") as f:
            await f.write(html_content)

        caption = (f"✅ *Notes ready!* ({len(all_notes)}/{len(topics)} topics)\n\n"
                   f"📚 Topics: {', '.join(topics)}\n🌐 Language: {language.title()}\n\n"
                   f"_Browser mein kholo!_ 📖\n\nMade by {CREDIT_TAG}")
        uploaded = await safe_send_doc(ctx.bot, chat_id, filepath, filename, caption)
        filepath.unlink(missing_ok=True)
        if uploaded:
            try: await msg.delete()
        ...
        await safe_edit(msg, "⏹️ *Stopped.*")
    except Exception as e:
        logger.error(f"Notes gen error: {e}", exc_info=True)
        await safe_edit(msg, f"❌ Error: `{str(e)[:200]}`")
    finally:
        active_tasks.pop(uid, None)
        notes_prefs.pop(uid, None)
