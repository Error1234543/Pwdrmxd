...
# Pyrogram bot client — no 50MB limit, no flood wait
pyro = PyroClient(
    "pyro_bot",
    api_id=API_ID,
    api_hash=API_HASH,
...
async def pyro_send_document(chat_id: int, dest: Path, fname: str, caption: str, fsize: int, prog_msg, ext: str) -> bool:
    start    = time.time()
    last_upd = [0.0]
    ...
    async def progress(current, total):
        ...
        if now - last_upd[0] > 4:
            ...
            try:
                await prog_msg.edit_text(
                    progress_box(fname, current, fsize, speed, elapsed, f"📤 Uploading {ext.upper()}")
                )
            except Exception:
                pass
    ...
    try:
        ...
        log.error(f"pyro_send_document: {e}")
        return False


async def pyro_send_video(chat_id: int, dest: Path, fname: str, caption: str, fsize: int, prog_msg) -> bool:
    start    = time.time()
    last_upd = [0.0]
    ...
    async def progress(current, total):
        ...
        if now - last_upd[0] > 4:
            ...
            try:
                await prog_msg.edit_text(
                    progress_box(fname, current, fsize, speed, elapsed, "📤 Uploading Video")
                )
            except Exception:
                pass
...
async def recv_file(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not items:
        await msg.edit_text("❌ Koi valid link nahi mila.")
        sessions[uid] = {"step": "wait_file"}
        return
    ...
    await msg.edit_text(
        ...
        f"📦 Total  : {total}\n"
        f"━━━━━━━━━━━━━━━━━━━━\n\n"
        f"📌 Kis number se start karna hai?\n"
...
async def recv_text(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if step == "wait_start_num":
        ...
        sessions[uid].update({"selected": selected, "start_from": start_num, "step": "wait_token"})
    ...
    if step == "wait_token":
        if len(text) < 50 or not text.startswith("eyJ"):
            ...
            return
        sessions[uid].update({"token": text, "step": "wait_quality"})
    ...
    if step == "wait_caption":
        sessions[uid].update({"caption_prefix": text, "step": "downloading"})
...
async def cb_quality(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    if not state:
        ...
        return
    sessions[uid].update({"quality": quality, "step": "wait_caption"})
        ...
        if ftype == "pdf":
            ...
            if ok and dest.exists() and dest.stat().st_size > 0:
                ...
                try:
                    ...
                    up_ok = await pyro_send_document(chat_id, dest, fname, caption, fsize, prog_msg, "pdf")
                    if up_ok:
        ...
        # ── VIDEO ──
            ...
            if ok and dest.exists() and dest.stat().st_size > 0:
                ...
                try:
                    ...
                    up_ok = await pyro_send_video(chat_id, dest, fname, caption, fsize, prog_msg)
                    if up_ok:
                        await prog_msg.edit_text(f"✅ Done: {name}")
                        ok_cnt += 1
                    else:
...
def main():
    if not BOT_TOKEN:   raise RuntimeError("BOT_TOKEN nahi hai!")
    if not WEBHOOK_URL: raise RuntimeError("WEBHOOK_URL nahi hai!")
    if not API_ID:      raise RuntimeError("API_ID nahi hai!")
    if not API_HASH:    raise RuntimeError("API_HASH nahi hai!")
    ...
    ptb.add_handler(MessageHandler(filters.Document.ALL, recv_file))
    ptb.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, recv_text))
    ptb.add_handler(CallbackQueryHandler(cb_quality, pattern=r"^q_\d+_\d+$"))
