...
def is_allowed_chat(chat_id, chat_type):
    """Personal chat always allowed for authorized users. Groups — only allowed list."""
...
def parse_questions(text):
    ...
    for part in parts:
        ...
        options, answer = [], None
        for line in lines[1:]:
            if re.match(r'^\(\d+\)', line): options.append(re.sub(r'^\(\d+\)\s*','',line))
            elif re.match(r'^\d+[.)]\s',line): options.append(re.sub(r'^\d+[.)]\s*','',line))
...
async def start(update: Update, ctx: ContextTypes.DEFAULT_TYPE):
    ...
    # ── Group check ──
    if not is_allowed_chat(chat_id, chat_type):
...
async def show_main_menu(update, ctx, uid, name):
    ...
    msg = (
        ...
        f"╚══════════════════════════╝\n"
        f"```\n"
        f"Aaja *{name}* bhai! {role}{expiry_text}\n\n"
        f"*Kya karna hai aaj?* 👇"
    )
...
#  QUIZ POLL
# ══════════════════════════════════════════════════════════════════════════════
async def handle_quiz_poll(update, ctx, text):
    questions = parse_questions(text)
    if not questions:
        await update.message.reply_text("❌ MCQ parse nahi hua!", parse_mode=ParseMode.MARKDOWN); return
    msg = await update.message.reply_text(f"✅ *{len(questions)} questions!*\n⏳ Polls ban rahe hain...", parse_mode=ParseMode.MARKDOWN)
    success = fail = 0
...
#  QUIZ HTML
# ══════════════════════════════════════════════════════════════════════════════
async def handle_quiz_html(update, ctx, text, title):
    questions = parse_questions(text)
    if not questions:
        await update.message.reply_text("❌ MCQ parse nahi hua!", parse_mode=ParseMode.MARKDOWN); return
    msg = await update.message.reply_text(f"⚙️ *{len(questions)} questions* se HTML ban rahi hai...", parse_mode=ParseMode.MARKDOWN)
    try:
        html_content = build_quiz_html(title, questions)
        # Use title as filename directly (user ne jo naam diya)
